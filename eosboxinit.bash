    ...
    start)
        ...
        for j in ${SERVICES[@]}; do
            if [[ $j = *:* ]]; then
                ...
                chroot $CHROOT /bin/su -c "/usr/local/bin/eosboxctl exec $service" > /dev/null 2>&1 &
                sleep $delay
            else
                chroot $CHROOT /bin/su -c "/usr/local/bin/eosboxctl exec $j" > /dev/null 2>&1 &
            fi
        done
        if [[ -e /etc/eosbox.rc ]]; then
            cp /etc/eosbox.rc /tmp/eosbox.rc
            chmod +x /tmp/eosbox.rc
            chroot $CHROOT /bin/su -c '/tmp/eosbox.rc' > /tmp/eosbox.rc.log 2>&1
            rm /tmp/eosbox.rc
        fi
        exit 0
    ;;
